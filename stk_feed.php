			...
			if((int)$auto_download["value"]=='1'){
				...
				if($empty!=0) {
					if($_SERVER['HTTP_HOST'] == 'linux.itvn.fr'){
							file_put_contents("/var/lib/mysql/imsrt/efrc_stk_inv_feed.txt",$get_file);
							if("08:15:00"<= date('H:i:s') && "09:15:00" >= date('H:i:s')){
								file_put_contents("/var/lib/mysql/imsrt/efrc_stk_inv_feed_".date('H:i:s').".txt",$get_file);
							}
						}
						else{
							file_put_contents("ftp_upload/efrc_stk_inv_feed.txt",$get_file);
							/*if("08:15:00"<= date('H:i:s') && "09:15:00" >= date('H:i:s')){
								file_put_contents("ftp_upload/efrc_stk_inv_feed_".date('H:i:s').".txt",$get_file);
							}	*/
						}
				}
				else {
						if($_SERVER['HTTP_HOST'] == 'linux.itvn.fr'){
							file_put_contents("/var/lib/mysql/imsrt/efrc_stk_inv_feed.txt",array());
							if("08:15:00"<= date('H:i:s') && "09:15:00" >= date('H:i:s')){
								file_put_contents("/var/lib/mysql/imsrt/efrc_stk_inv_feed_".date('H:i:s').".txt",array());
							}	
						}
						else{
							file_put_contents("ftp_upload/efrc_stk_inv_feed.txt",array());	
							/*if("08:15:00"<= date('H:i:s') && "09:15:00" >= date('H:i:s')){
								file_put_contents("ftp_upload/efrc_stk_inv_feed_".date('H:i:s').".txt",array());
							}*/
						}
				}
			}
