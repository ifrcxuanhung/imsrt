...
function image_upload(field, thumb) {
    ...
    $('#dialog').dialog({
        ...
        //        close: function(event, ui) {
        //            $.ajax({
        //                url: $admin_url + 'product/check_file',
        //            });
        //        },
        bgiframe: false,
        width: 800,
...
function image_upload_multiple(field, thumb, act) {
    window.KCFinder = {};
    window.KCFinder.callBackMultiple = function(url) {
        $('#dialog').dialog('close');
        var $thumb = new Array();
        for (var i = 0; i < url.length; i++)
            $thumb.push(url[i].replace('assets/upload', 'assets/upload/thumbs'));
        $('#' + thumb).replaceWith('<img src="' + $thumb[0] + '" alt="" id="' + thumb + '" />');
        for (var i = 1; i < $thumb.length; i++)
            $('#thumb').before('<img src="' + $thumb[i] + '" alt="" id="' + thumb + i + '" />');
        $('#image').val(url);
        window.KCFinder = null;
    };
...
function clearImage() {
    $('#thumb').parent('.image').find('img:not(#thumb)').remove();
    $('#thumb').attr('src', $base_url + 'assets/images/no-image.jpg');
    $('#image').attr('value', '');
}
