define([
    ...
    'backbone'
        ...
        var jq_realtimeView = Backbone.View.extend({
            el: $(".main-container"),
            initialize: function(){
		   ...
		   var codeint = $(".jq_table3").attr('codeint');
            ...
            $("#jqGrid1").jqGrid({
               // url: 'ajax/jq_efrc_currency_data',
			    url: $base_url+'ajax/jq_loadrealtime1',
			...
			$("#jqGrid2").jqGrid({
               // url: 'ajax/jq_efrc_currency_data',
			    url: $base_url+'ajax/jq_loadrealtime2',
			...
			$("#jqGrid3").jqGrid({
               // url: 'ajax/jq_efrc_currency_data',
			    url: $base_url+'ajax/jq_loadrealtime3',
				editurl: $base_url+'ajax/edit_del_add_jq_loadtable?jq_table='+jq_table3,
			
                mtype: "POST",
                datatype: "json",
				postData:{jq_table3:jq_table3,filter_get_all:filter_get_all,codeint:codeint},
                page: 1,
                colModel:column3,
				//loadonce: true,
				loadtext: "Loading...",
			...
			$("#jqGrid4").jqGrid({
               // url: 'ajax/jq_efrc_currency_data',
			    url: $base_url+'ajax/jq_loadrealtime4',
			...
			$("#jqGrid5").jqGrid({
               // url: 'ajax/jq_efrc_currency_data',
			    url: $base_url+'ajax/jq_loadrealtime5',
				editurl: $base_url+'ajax/edit_del_add_jq_loadtable?jq_table='+jq_table5,
			
                mtype: "POST",
                datatype: "json",
				postData:{jq_table5:jq_table5,filter_get_all:filter_get_all,codeint:codeint},
                page: 1,
                colModel:column5,
				//loadonce: true,
				loadtext: "Loading...",
			...
			function formatLink(cellValue, options, rowObject) {
				console.log(rowObject.idx_mother);
				// cellValue is value of this field
				// rowObject is all data
                return "<a href='"+$base_url+"indice/" + rowObject.idx_mother + "' class='link_overview'>" + cellValue.substring(0, 25) + "</a>";
            };
            ...
            },
			{
				...
				beforeShowForm: function(form) {
                  ...
                  // TODO: change parentWidth and parentHeight in case of the grid
                  //       is larger as the browser window
                //  dlgDiv[0].style.top = Math.round((parentHeight-dlgHeight)/8) + "px";
                //  dlgDiv[0].style.left = Math.round((parentWidth-dlgWidth)/2) + "px";
              }
			}
			);
            ...
            },
			{
				...
				beforeShowForm: function(form) {
                  ...
                  //       is larger as the browser window
                 // dlgDiv[0].style.top = Math.round((parentHeight-dlgHeight)/2) + "px";
                  //dlgDiv[0].style.left = Math.round((parentWidth-dlgWidth)/8) + "px";
              }
			}
			);
            ...
            // BEGIN CHART 
                ...
                for (var i = 1; i < data1.length; ++i){
					...
					if(data1[i].time >= '09:00:00' && data1[i].time <= '14:45:00'){
					  // if(data1[i+1].idx_plast.length > 0){
    					//	var value = ;
							var time = data1[i].time.slice(0,5);
    						res.push({'time':time,'value': parseFloat(data1[i].idx_last).toFixed(2)})
                       /// }
					}
                } 
               // console.log(res);
