define([
    ...
    'backbone'
        ...
        var tableView = Backbone.View.extend({
            ...
            events: {
                 ...
                 "click .runquerylocal": "actionRunQueryLocal",
				 "click .runqueryhost": "actionRunQueryHost",
				 "click .selectquery": "actionSelectQuery",
				 ...
				 "click .show-all-query": "actionShowQuery",
			...
			actionSelectQuery: function(event) {
                var $this = $(event.currentTarget);
			
                //console.log($this.attr("tablename"));
                $.ajax({
                    url: $base_url + "ajax/show_select_query",
                    type: "POST",
                    data: {},
                    async: false,
                    success: function(response) {
 ...
 //                console.log(table.find('tr.active').children('.checkboxes').value());
			...
			actionRunQueryLocal: function(event){
			   var table = $('#table');
               var dataArr = [];
               $('#table tr.active').each(function(){
                   // alert($(this).find('.checkboxes').val());// just to see the rowid's -> $(this).closest('tr[id]').attr('id')
               ...
               $.ajax({
                      type : "POST",
                      url : $base_url + 'ajax/runQueryLocal',
                      data : {dataArr:dataArr},
                      success: function(res) {
						  response = JSON.parse(res);
                       //console.log(res);
 ...
 //                console.log(table.find('tr.active').children('.checkboxes').value());
			...
			actionRunQueryHost: function(event){
			   var table = $('#table');
               var dataArr = [];
               $('#table tr.active').each(function(){
                   // alert($(this).find('.checkboxes').val());// just to see the rowid's -> $(this).closest('tr[id]').attr('id')
               ...
               $.ajax({
                      type : "POST",
                      url : $base_url + 'ajax/runQueryHost',
                      data : {dataArr:dataArr},
                      success: function(res) {
 ...
 //                console.log(table.find('tr.active').children('.checkboxes').value());
			...
			actionShowQuery: function(event) {
                var $this = $(event.currentTarget);
				var validate = $this.attr("keys_value");
                //console.log($this.attr("tablename"));
                $.ajax({
                    url: $base_url + "ajax/show_query",
                    type: "POST",
                    data: {table_name: $this.attr("table_name"), keys_value: $this.attr("keys_value")},
                    async: false,
                    success: function(response) {
...
//                                }  
				   ...
				   console.log($('#text_note').val());
                  $(".toolbar").html( '<div style="margin-top:3px; font-size:13px !important;">'+$('#text_note').val()+"</div>");
               });
