...
class Composition_model extends CI_Model {
    ...
    function Composition_Insert($compo) {
        $compo["date"] = '2010-09-17';
        $compo["time"] = $this->time;
...
//Xoa Cache
    ...
    function cal_composition($cache, $h, $m, $s) {
            ...
            foreach ($composition as $compo => $value) {
                foreach ($sp as $key => $row) {
                    if ($row['specs_id'] == $value['specs_id']) {
                        ...
                        $sql = "Update idx_composition set stk_wgt = " . $wgt . ", time = '" . $h . ":" . $m . ":" . $s . "', date = '" . date('Y-m-d') . "' Where id = " . $value['id'];
                        $this->db->query($sql);
                        break;
                    }
                }
        ...
        if ($cache)
            foreach ($cache as $specs) {
                ...
                //echo $last_r;
                $sql = "Update idx_specs SET idx_last = '$last_r', time = '" . $h . ":" . $m . ":" . $s . "', idx_mcap = " . $p[0]['idx_mcap'] . " , idx_dcap = " . $p[0]['idx_dcap'] . "   WHERE id = {$specs['id']} and calculs = 1 ";
                $this->db->query($sql);
            }
    ...
    public function getListIDX_code() {
        $sql = "SELECT DISTINCT {$this->table}.idx_code
                FROM {$this->table}, idx_ref, stk_ref, cur_rates
                WHERE {$this->table}.idx_code = idx_ref.idx_code
                AND stk_ref.stk_code = {$this->table}.stk_code
                AND idx_ref.calculs=1
                AND {$this->table}.stk_curr = cur_rates.currency order by {$this->table}.idx_code";
        return $this->db->query($sql)->result_array();
    }

    public function find($id = '', $code = '', $date) {
        $and = '';
        if (is_numeric($id) == TRUE) {
            $and.= " AND {$this->table}.id = " . $id;
        }
        if ($code == '') {
            $codes = self::getListIDX_code();
            $code = $codes[0]['idx_code'];
        }
        $and.= " AND {$this->table}.idx_code = '" . $code."' ";
        if ($date != '') {
            $and.= " AND {$this->table}.date = '" . $date."' ";
        }
        $sql = "SELECT DISTINCT {$this->table}.id, {$this->table}.`date`, {$this->table}.stk_code, stk_ref.stk_name,
                {$this->table}.idx_code, idx_ref.idx_name, {$this->table}.stk_shares_idx, {$this->table}.stk_float_idx,
                {$this->table}.stk_capp_idx, {$this->table}.stk_price, {$this->table}.`time`, {$this->table}.stk_mcap_idx,
                cur_rates.currency, {$this->table}.stk_wgt
                FROM {$this->table}, idx_ref, stk_ref, cur_rates
                WHERE {$this->table}.idx_code = idx_ref.idx_code
                AND stk_ref.stk_code = {$this->table}.stk_code
                AND idx_ref.calculs=1
                AND {$this->table}.stk_curr = cur_rates.currency";
        $sql .= $and;
        return $this->db->query($sql)->result_array();
    }
    ...
    public function loadListStkcodeFromStk_ref() {
        $sql = "SELECT DISTINCT stk_ref.stk_code, stk_ref.stk_name
                FROM stk_ref;";
        return $this->db->query($sql)->result_array();
    }

    public function loadListCurrdateFromIdx_calendar() {
        $sql = "SELECT DISTINCT idx_calendar.date
                FROM idx_calendar;";
        return $this->db->query($sql)->result_array();
    }

    public function loadListIdxcodeFromIdx_ref() {
        $sql = "SELECT DISTINCT idx_ref.idx_code, idx_ref.idx_name
                FROM idx_ref;";
        return $this->db->query($sql)->result_array();
    }

    public function loadListIdxeventcodeFromEvt_ref() {
        $sql = "SELECT DISTINCT evt_ref.evt_code, evt_ref.evt_name
                FROM evt_ref;";
        return $this->db->query($sql)->result_array();
    }

    public function loadListMarcodeFromMar_ref() {
        $sql = "SELECT DISTINCT mar_ref.mar_code as `stk_market`, mar_ref.mar_name as `market_name`
                FROM mar_ref;";
        return $this->db->query($sql)->result_array();
    }

    public function loadListSeccodeFromSec_ref() {
        $sql = "SELECT DISTINCT sec_ref.sec_code, sec_ref.sec_name
                FROM sec_ref;";
        return $this->db->query($sql)->result_array();
    }

    public function loadListCurrencyFromCur_rates() {
        $sql = "SELECT DISTINCT cur_rates.currency as `stk_curr`
                FROM cur_rates;";
        return $this->db->query($sql)->result_array();
    }
