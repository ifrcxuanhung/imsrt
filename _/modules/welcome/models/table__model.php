...
class Table__model extends CI_Model {
    ...
    function get_headers ($table='',$field='') {
        ...
        $sql = "select *
                ...
                where (`active` <= ".(($this->session->userdata_vnefrc('user_level')) ? $this->session->userdata_vnefrc('user_level') : 0). " or `active` =1) and (`active` !=0) and (`table` ='".$table."') order by `order` asc ";
	...
	function get_tab ($tab='') {
		...
		$sql = "select *
				...
				where (table_name = '$tab' OR table_name='efrc_$tab' ) ";
    ...
    function get_summary_efrc ($table='') {
		$sql = "select *
                from efrc_summary 
                where `tab` = '$table'";
		 return  $this->db->query($sql)->row_array();
    ...
    function append_date($field = '') {
        if($field == '') {
            ...
            $html = '<div class="custom-hung-date  input-group date date-picker margin-bottom-5">';
            $html .= '<input type="text" id="date_'.strtolower($field).'_start" name="date_'.strtolower($field).'_start" placeholder="Choose..." class=" form-control form-filter input-sm date-picker" data-date-format="yyyy-mm-dd" autocomplete="off">';
            ...
            $html .= '<input type="text" id="date_'.strtolower($field).'_end" name="date_'.strtolower($field).'_end" placeholder="Choose..." class=" form-control form-filter input-sm date-picker" data-date-format="yyyy-mm-dd" autocomplete="off">';
            $html .= '<span class="input-group-btn">';
            $html .= '<button class="btn btn-icon-only default" type="button"><i class="fa fa-calendar"></i></button>';
            $html .= '</span>';
            $html .= '</div>';
	 ...
	 function append_range($field = '') {
        if($field == '') {
			...
			$html .='<input type="text" placeholder="From" name="'.strtolower($field).'_from" class="custom-hung form-control form-filter input-sm">';
            
			$html .= '<input type="text" placeholder="To" name="'.strtolower($field).'_to" class="custom-hung form-control form-filter input-sm">';
			$html .= '</div>';
            return $html;
        }
    }
    function append_select($field = '',$arr='', $format_n='', $value_filter='') {
        if($field == '') {
            ...
            $list_data = $this->db->query("select 
												CASE
												WHEN `$field` IS NULL OR TRIM(`$field`) <=''
												THEN ''
												ELSE `$field`
												END as `$field` from $arr group by CASE WHEN `$field` IS NULL OR TRIM(`$field`) <='' THEN '' ELSE `$field` END  order by `$field` asc;")->result_array();
            ...
            foreach ($list_data as $item) {
                if($item[$field] !='' ){
    				if((strpos(strtolower($format_n),'decimal')!==false)&&$item[$field]!=''){
    					...
    					$str = ($start!==false && $end!==false) ? efrcval(substr($format_n,$start,  $end - $start)) : 0;
    			...
    			//	$item[$field] = $item[$field]!='' ? $item[$field] :'empty';
                    ...
                    $html .= '<option value="'.$item[$field].'" selected="selected">'.$value.'</option>';
					 ...
					 $html .= '<option value="is_null">'.$item[$field].'</option>';
				}
            }
            return $html;
        }
    ...
    function append_date_editon($field = '',$value ='',$readonly = '',$primary=0) {
        if($field == '') {
			...
			$read = $readonly == '1' ? 'readonly="readonly"' : '' ;
            $html = '<div class="input-icon"><i class="fa fa-calendar"></i><input type="text" data-date-viewmode="years" id="'.strtolower($field).'" name="'.strtolower($field).'"'.$read.'  data-date-format="yyyy-mm-dd" data-date="'.$value.'" value="'.$value.'" size="16" class="form-control date-picker-modal"></div>';
            return $html;
        }
    }
    ...
    function get_composition_table($table, $idx_code){
        $sql = 'SELECT field FROM sys_format WHERE `table`="'.$table.'" and active ="1"';
        ...
        $columm = substr($columms,0,-2);
        //print_R($columm);exit;
        $sqlmain = "SELECT ".$columm. " 
                FROM {$table}
                GROUP BY {$table}.stk_code
                ORDER BY stk_wgt DESC;";
        $info = $this->db->query($sqlmain)->result_array();
        if (count($info) > 0) {
