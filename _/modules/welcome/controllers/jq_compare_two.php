<?php
class Jq_compare_two extends Welcome{
    ...
    public function index(){
		...
		$sql_truncate = "truncate idx_composition_temp;";
		$this->db->query($sql_truncate);
		$sql_truncate2 = "truncate idx_specs_temp;";
		$this->db->query($sql_truncate2);
		$sql_drop1 = "DROP TABLE IF EXISTS idx_composition_compare_open;";
		...
		if(isset($_REQUEST['actexport4'])&&($_REQUEST['actexport4']=='exportCsv4'||$_REQUEST['actexport4']=='exportTxt4')){
		 ...
		 if(isset($_REQUEST['currCompostion'])&&($_REQUEST['currCompostion']=='VND')){
			$currencyCompostion =" WHERE right(a.`idx_code`,3)='VND'";
		}
		else 
			$currencyCompostion = "";
		if(isset($_REQUEST['currSpecs'])&&($_REQUEST['currSpecs']=='VND')){
		...
		$composition_database_other = $this->db7->select("idx_code,stk_code,stk_shares_idx,stk_mcap_idx,stk_dcap_idx,stk_price,date,time")->get("idx_composition_rt")->result_array();
		...
		$this->db->insert_batch('idx_composition_temp', $composition_database_other);
		$this->db->insert_batch('idx_specs_temp', $specs_database_other);
		
		$table_common ="create table idx_composition_compare_open (SELECT a.idx_code,a.stk_code, a.stk_shares_idx, b.stk_shares_idx as stk_shares_idx_rt, ABS(a.stk_shares_idx - b.stk_shares_idx) as diff1,(CASE 
      ...
      END) AS stt3, a.stk_price,b.stk_price as stk_price_rt , ABS(a.stk_price - b.stk_price) as diff4,(CASE 
         ...
         ELSE 1
      END) AS stt4, a.date FROM idx_composition_temp as a INNER JOIN idx_composition_rt as b ON a.idx_code=b.idx_code and a.stk_code=b.stk_code {$currencyCompostion}); ";
	 ...
	 /* $table_common ="(SELECT a.idx_code,a.stk_code, a.stk_shares_idx, b.stk_shares_idx as stk_shares_idx_ims, ABS(a.stk_shares_idx - b.stk_shares_idx) as diff1,(CASE 
      ...
      END) AS stt3, a.stk_price,b.stk_price as stk_price_ims , ABS(a.stk_price - b.stk_price) as diff4,(CASE 
         ...
         ELSE 1
      END) AS stt4, a.date FROM idx_composition_temp as a INNER JOIN idx_composition_rt as b ON a.idx_code=b.idx_code and a.stk_code=b.stk_code) A";*/
      ...
      END) AS stt4, a.idx_pclose,b.idx_pclose as pclose_ims , ABS(a.idx_pclose - b.idx_pclose) as diff5,(CASE 
         ...
         ELSE 1
      END) AS stt5 FROM idx_specs_temp as a INNER JOIN idx_specs_rt as b ON a.idx_code=b.idx_code ) A";*/
		...
		foreach($column as $k=>$val_column){
			if($val_column['searchoptions'] == 'select'){
				...
				$query = $this->db->get("idx_composition_compare_open");
		...
		foreach($column2 as $k2=>$val_column2){
			if($val_column2['searchoptions'] == 'select'){
				...
				$query2 = $this->db->get("idx_specs_compare_open");
		...
		$this->table->count_top_composition("idx_composition_compare_open");
		$this->table->count_top_specs("idx_specs_compare_open");
	
	 

		
		$this->template->write_view('content', 'jq_compare_two',$this->data);
	...
	public function edit_del_add_jq_compare_two(){
	...
	function export() {
      ...
      END) AS stt3, a.stk_price,b.stk_price as stk_price_ims , ABS(a.stk_price - b.stk_price) as diff4,(CASE 
         WHEN (a.stk_price - b.stk_price) = 0 THEN 0
         ELSE 1
      END) AS stt4, a.date FROM idx_composition_temp as a INNER JOIN idx_composition_rt as b ON a.idx_code=b.idx_code and a.stk_code=b.stk_code)";
	...
	function bodyReport2($content,$tab_name){
      ...
      END) AS stt4, a.idx_pclose,b.idx_pclose as pclose_ims , ABS(a.idx_pclose - b.idx_pclose) as diff5,(CASE 
         WHEN (a.idx_pclose - b.idx_pclose) = 0 THEN 0
         ELSE 1
      END) AS stt5 FROM idx_specs_temp as a INNER JOIN idx_specs_rt as b ON a.idx_code=b.idx_code )";
		}	
		else {
