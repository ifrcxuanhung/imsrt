...
class Table extends Welcome{
    ...
    public function index($table ='summary', $category = "") {
		...
		if(!isset($arr_table_sys["table_name"])){
			 $this->template->write_view('content', 'table/error', $this->data);
			...
			// neu code=INDVNINDEX lay name lam tieu de
		if(isset($_GET['code'])){
		 $this->data->namebycode = $this->db->select('name')->where('codeint',$_GET['code'])
         ->get('efrc_ins_ref')->row_array();
		}
    ...
    function export() {
		...
		$table_sys = isset($arr_table_sys["table_name"]) ? $arr_table_sys["table_name"] : $table;
		$headers = $this->table->get_headers($table_sys);
        // select columns
        $where = "where 1=1";
        $aColumns = array();
	...
	function bodyReport($content,$tab_name){
		...
		$headers = $this->table->get_headers($tab['table_name'],$tab['query']);
		$this->load->dbutil();
