...
class Cur_linux extends Welcome{
    ...
    public function index() {
		...
		while ($i<$k) {
			...
			if (file_exists('/var/lib/mysql/imsrt/efrc_cur_blg_feed.txt')) {
				$sql = "truncate table cur_feed_rt_temp;";
				$this->db->simple_query($sql);
				$command = "load data infile 'efrc_cur_blg_feed.txt' into table `cur_feed_rt_temp` character set utf8 fields terminated by  '\t'  lines terminated by  '\r\n' (code, currency,tos,curr_conv,date,time, pclose, `change`, `percent`);";
				$this->db->query($command);
// INSERT 2 ROWS FOR VND======================================
                $this->db->query("set @USDJPY= (select curr_conv from cur_feed_rt_temp where `code`='USDJPY');");
                $this->db->query("set @USDVND= (select curr_conv from cur_feed_rt_temp where `code`='USDVND');");
                $this->db->query("set @USDEUR= (select curr_conv from cur_feed_rt_temp where `code`='USDEUR');");
...
// END======================================================
		...
		set a.last=b.curr_conv,a.time=b.time, a.date=b.date,a.pclose=b.pclose,a.change=b.change,a.percent=b.percent, a.updateflag=1;";
				$this->db->query($sql_update);
				$sql_insert ="INSERT INTO cur_feed_rt(`symbol`,`date`,`time`,`last`,`code`,`updateflag`,`pclose`,`change`,`percent`)
				SELECT 
					b.`code`,b.date,b.time,b.curr_conv, CONCAT('CUR',b.`code`) as codeint,1,b.`pclose`, b.`change`, b.`percent`
				FROM
...
// UPDATE 3 ROWS FOR VND======================================              
                $this->db->query("set @USDJPY= (select last from cur_feed_rt where `symbol`='USDJPY');");
                $this->db->query("set @USDVND= (select last from cur_feed_rt where `symbol`='USDVND');");
                $this->db->query("set @USDEUR= (select last from cur_feed_rt where `symbol`='USDEUR');");
                $this->db->query("set @JPYVND= @USDVND/@USDJPY;");
                $this->db->query("set @EURVND= @USDVND/@USDEUR;");
                ...
                (select time from cur_feed_rt where `symbol`='USDVND') time, @JPYVND as last 
                UNION
                select 'EURVND' as `code`,'EUR' as currency,'VND' as tos,(select max(date) from cur_feed_rt) as date,(select time 
                from cur_feed_rt where `symbol`='USDVND') time, @EURVND as last) as b 
                set a.last=b.last,a.date=b.date, a.time=b.time where a.symbol=b.`code`;");
// END======================================================                          	
			}
			else {
				$sql = "truncate table cur_feed_rt_temp;";
	...
	set a.last=b.rt1_last,a.time=b.rt1_lastt, a.date=b.rt1_lastd;";
				...
				$this->db->query($sql_insert);*/
				//efrc_ind_vn
			if (file_exists('/var/lib/mysql/imsrt/efrc_ind_vn_feed.txt')) {
				$sql = "truncate table efrc_ind_vn;";
				$this->db->simple_query($sql);
				$command = "load data infile 'efrc_ind_vn_feed.txt' into table `efrc_ind_vn` character set utf8 fields terminated by  '\t'  lines terminated by  '\r\n' (`name`,`index`,`percent`,`change`,`codeint`);";
				$this->db->query($command);				
	...
	set a.last=b.`index`,a.time=b.time, a.date=b.date, a.`percent`=b.`percent`,a.`change`=b.`change`, a.updateflag=1; ";
				...
				//cmd
			if (file_exists('/var/lib/mysql/imsrt/efrc_cmd_quotenet.txt')) {
				$sql = "truncate table cmd_feed_rt_tempt;";
				$this->db->simple_query($sql);
				$command = "load data infile 'efrc_cmd_quotenet.txt' into table `cmd_feed_rt_tempt` character set utf8 fields terminated by  '\t'  lines terminated by  '\r\n' (`symbol`,`name`,`date`,`time`,`last`,`percent`,`change`,`pclose`,`code`);";
				$this->db->query($command);
