...
class Vfdb_auth extends MY_Controller {
    ...
    function login() {
        ...
        $module_login = $this->session->userdata_vnefrc('module_login');
        if ($this->form_validation->run() == true) {
            ...
            if ($this->vfdb_ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember)) {
                $response['url'] = $this->session->userdata_vnefrc('url_login');
                if($this->input->is_ajax_request()){
                    echo json_encode($response);
                }
                redirect($this->session->userdata_vnefrc('url_login'), 'refresh');
            } else {
    ...
    function change_password() {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $identity = $this->session->userdata_vnefrc($this->config->item('identity', 'vfdb_ion_auth'));
