	...
	function check_user_pass($user,$pass){
	 ...
	 public function table_get($table = '', $type='xml') {
        ...
        if ($this->check_user_pass($user,$pass) != 1) {
            $this->session->set_userdata_vnefrc('table', $table);
            $this->session->set_userdata_vnefrc('page', $this->get('page'));
            $this->session->set_userdata_vnefrc('method', $this->get('method'));
            Redirect('webservices/login');
        } else {
            $page = $this->get('page') != '' ? $this->get('page') : $this->session->userdata_vnefrc('page');
			
            if($this->get('method') != '') {
                $method = $this->get('method');
            } elseif($this->session->userdata_vnefrc('method') != '') {
                $method = $this->session->userdata_vnefrc('method');
            } else {
                $method = '';
            }
            if($this->get('code') != '') {
            ...
            if ($table != '' && $table != 'format') {
                if($type =='xml'){
                    ...
                    $this->session->set_userdata_vnefrc('method', '');
