...
class Category extends Admin {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if (isset($this->data->list_category) == TRUE && $this->data->list_category != '' && is_array($this->data->list_category) == TRUE) {
                foreach ($this->data->list_category as $key => $value) {
                    ...
                    if ($value->status == 1) {
                        $aaData[$key][] = '<ul style="text-align: center;"><a style="color: green;" class="category-active" category_id="' . $value->category_id . '" href="javascript: void(0);">Enable</a></ul>';
                    } else {
                        $aaData[$key][] = '<ul style="text-align: center;"><a style="color: red;" class="category-active" category_id="' . $value->category_id . '" href="javascript: void(0);">Disable</a></ul>';
                    }
                    ...
                    $aaData[$key][] = '<ul class="keywords" style="text-align: center;"><li class="green-keyword"><a class="with-tip" href="' . admin_url() . 'category/edit/' . $value->category_id . '">' . trans('bt_edit', 1) . '</a></li>
                                   <li class="red_fx_keyword '. ($value->parent_id == 0 ? 'bt-delete-admin' : '') .'"><a title="" class="with-tip action-delete ' . ($value->parent_id == 0 ? 'is_admin' : '') . '" category_id="' . $value->category_id . '" href="#">' . trans('bt_delete', 1) . '</a></li></ul>';
                }
            }
            $output = array(
                "aaData" => $aaData
    ...
    function add() {
        ...
        $this->data->sort_order = $this->category_model->detect_sort_order();
        //get all category
        $list_category = $this->category_model->list_category_fix();
        // data for dropdown list parent category
    ...
    function edit($id = "") {
        ...
        if (isset($this->data->list_language) && is_array($this->data->list_language)):
            foreach ($this->data->list_language as $value) :
                if ($value['code'] == $this->data->default_language['code']) {
                    $this->form_validation->set_rules('name_' . $value['code'], trans("name", 1), 'required');
                }
            endforeach;
        endif;
        // run validate
