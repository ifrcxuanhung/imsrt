...
class Media extends Admin {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ((isset($this->data->list_media) == TRUE) && ($this->data->list_media != '') && (is_array($this->data->list_media) == TRUE)) {
                ...
                foreach ($this->data->list_media as $key => $value) {
                    ...
                    if ($value['status'] == 1) {
                        $aaData[$key][] = '<ul style="text-align: center;"><a style="color: green;" class="media-active" media_id="' . $value['media_id'] . '" href="javascript: void(0);">Enable</a></ul>';
                    } else {
                        $aaData[$key][] = '<ul style="text-align: center;"><a style="color: red;" class="media-active" media_id="' . $value['media_id'] . '" href="javascript: void(0);">Disable</a></ul>';
                    }
                    $aaData[$key][] = '<a class="fancybox" style="display: block;width: 35px" href="' . (isset($value['image']) ? base_url() . str_replace(base_url(), "", $value['image']) : base_url() . 'assets/upload/images/no-image.jpg') . '" title="' . $value['title'] . '">
                                        <img class="thumbnails" src="' . (isset($value['thumb']) ? base_url() . str_replace(base_url(), "", $value['thumb']) : base_url() . 'assets/upload/images/no-image.jpg') . '" /></a>';
                    $aaData[$key][] = '<ul class="keywords" style="text-align: center;"><li class="green-keyword"><a class="with-tip" href="' . admin_url() . 'media/edit/' . $value['media_id'] . '">' . trans('bt_edit', 1) . '</a></li>
                                   <li class="red_fx_keyword"><a class="with-tip action-delete " media_id="' . $value['media_id'] . '" href="#">' . trans('bt_delete', 1) . '</a></li></ul>';
                }
                $output = array(
                    "aaData" => $aaData
                );
    ...
    function add() {
        ...
        $this->data->sort_order = $this->media->detect_sort_order();
