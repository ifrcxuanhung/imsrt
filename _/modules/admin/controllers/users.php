...
class Users extends Admin {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ((isset($this->data->users) == TRUE) && ($this->data->users != '') && (is_array($this->data->users) == TRUE)) {
                ...
                foreach ($this->data->users as $key => $value) {
                    ...
                    $aaData[$key][] = '<p style="text-align: left;">' . ++$id . '</p>';
                    $aaData[$key][] = $value->first_name;
                    $aaData[$key][] = $value->last_name;
                    $aaData[$key][] = $value->email;
                    $aaData[$key][] = isset($group['name']) ? $group['name'] : '';
                    if ($value->active == '1') {
                        $aaData[$key][] = '<ul style="text-align: center;"><a style="color: green;" class="action-change-active" user_id="' . $value->id . '" href="javascript: void(0);">Active</a></ul>';
                    } else {
                        $aaData[$key][] = '<ul style="text-align: center;"><a style="color: red;" class="action-change-active" user_id="' . $value->id . '" href="javascript: void(0);">Inactive</a></ul>';
                    }
                    $aaData[$key][] = '<ul class="keywords" style="text-align: center;">
                                            <!--li class="green-keyword"><a class="with-tip" href="' . admin_url() . 'users/services/' . $value->id . '">Services</a></li-->
                                            <li class="green-keyword"><a class="with-tip" href="' . admin_url() . 'users/edit/' . $value->id . '">' . trans("bt_edit", 1) . '</a></li>
                                            <li class="red_fx_keyword"><a class="with-tip action-delete" user_id="' . $value->id . '" href="javascript: void(0);">' . trans("bt_delete", 1) . '</a></li></ul>';
                }
                $output = array(
                    "aaData" => $aaData
                );
    ...
    function chang_active() {
        ...
        if ($this->input->is_ajax_request()) {
            echo self::_change_active($this->input->post('id'), $this->input->post('text'));
        }
    }
    
    public function _change_active($id = NULL, $text = NULL) {
        if (!is_numeric($id)) {
            return FALSE;
        }
        $sql = "UPDATE user
                SET user.active = IF (user.active = 1, 0, 1)
                WHERE user.id = '{$id}'";
        $this->db->query($sql);
        $text = ($text == "Active") ? "Inactive" : "Active";
        return $text;
    }
