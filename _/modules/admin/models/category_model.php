...
class category_model extends CI_Model {
    ...
    function add($data = NULL, $list_language = NULL) {
        if (is_array($data) == TRUE && is_array($list_language) == TRUE) {
            $data_category = array(
                'category_code' => utf8_convert_url(strtolower($data['category_code'])),
                'image' => isset($data['image']) ? str_replace(base_url(), "", $data['image']) : '',
...
// return true;
    ...
    function edit($data = NULL, $list_language = NULL, $id = NULL) {
        if (is_array($data) == TRUE && is_array($list_language) == TRUE && is_numeric($id) == TRUE) {
            $data_category = array(
                'category_code' => utf8_convert_url(strtolower($data['category_code'])),
                'image' => isset($data['image']) == TRUE ? str_replace(base_url(), "", $data['image']) : '',
                'sort_order' => $data['sort_order'],
