...
class Page_model extends CI_Model {
    ...
    public function addPage($data = array(), $list_language = NULL) {
        $info = array(
            'name' => htmlspecialchars($data['name']),
            'layout_id' => $data['layout_id'],
            'code' => utf8_convert_url(strtolower($data['name'])),
    ...
    function editPage($data = array(), $list_language = array(), $id = "") {
    ...
    public function _check_exist($id = "", $lang_code = "") {
        if (!is_numeric($id)) {
            return FALSE;
        }
        $this->db->where('page_id', $id);
