        ...
        if ($module != 'auth') {
            ...
            try {
                if ($this->Zend_Acl->isAllowed($role, $module . ':' . $controller, $action) == FALSE) {
                    $this->CI->session->set_userdata_vnefrc('access', 'You do not have permission to access <strong>' . $controller . ' ' . $action . '</strong>, please refer to your system administrator.');
                    $link = base_url();
                    if ($module == 'admin') {
