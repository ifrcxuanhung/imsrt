...
class Profile extends Generic {
	...
	function __construct() {
		...
		if(!empty($_SESSION['imsrt']['forcePwUpdate']))
		...
		// Save the username
		$this->username = !empty($_SESSION['imsrt']['username']) ? $_SESSION['imsrt']['username'] : _('Guest');
		...
		if (!empty($_FILES['uploadAvatar'])) {
			...
			if (empty($k)) {
				...
				$uploadfile = $uploaddir . md5($_SESSION['imsrt']['user_id'] . $_SESSION['imsrt']['email']) . '.' . pathinfo($_FILES['uploadAvatar']['name'], PATHINFO_EXTENSION);
				if (move_uploaded_file($_FILES['uploadAvatar']['tmp_name'], $uploadfile)) {
					...
					$_SESSION['imsrt']['gravatar'] = parent::get_gravatar($_SESSION['imsrt']['email'], true, 26);
				} else {
					$this->error = sprintf('<div class="alert alert-warning">%s</div>', _('Sorry, that file is not accepted.') );
				}
			}
	...
	private function determineGuest() {
		...
		if ( !empty($_SESSION['imsrt']['user_id']) && empty($_GET['uid']) )
			$this->user_id = $_SESSION['imsrt']['user_id'];
		...
		$this->guest = !( !empty($_SESSION['imsrt']['user_id']) && $_SESSION['imsrt']['user_id'] == $this->user_id );
	...
	private function updateEmailorPw() {
		...
		if(!empty($_SESSION['imsrt']['forcePwUpdate'])) unset($_SESSION['imsrt']['forcePwUpdate']);
