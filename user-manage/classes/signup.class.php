...
class SignUp extends Generic {
	...
	public function getPost($var) {
		...
		foreach ($socialLogin as $value) {
			if( !empty($_SESSION['imsrt'][$value . 'Misc']) ) {
				$misc = $_SESSION['imsrt'][$value . 'Misc'];
			}
		}
	...
	private function register() {
		...
		if(empty($this->error)) {
			...
			//$_SESSION['imsrt']['username'] = $this->settings[$this->username_type];

			/* Apply default user_level */
			//$_SESSION['imsrt']['user_level'] = unserialize(parent::getOption('default-level'));

			if ( $requireActivate )
				$_SESSION['imsrt']['activate'] = 1;

			//$_SESSION['imsrt']['gravatar'] = parent::get_gravatar($this->settings['email'], true, 26);
...
//echo "<pre>";print_r($params);exit;
			...
			//$_SESSION['imsrt']['user_id'] = $user_id;

			/* Social integration. */
			if ( !empty($_SESSION['imsrt']['facebookMisc']) ) {
				$link = 'facebook';
				$id = $_SESSION['imsrt']['facebookMisc']['id'];
			}

			if ( !empty($_SESSION['imsrt']['openIDMisc']) ) {
				$link = $_SESSION['imsrt']['openIDMisc']['type'];
				$id = $_SESSION['imsrt']['openIDMisc'][$link];
			}

			if ( !empty($_SESSION['imsrt']['twitterMisc']) ) {
				$link = 'twitter';
				$id = $_SESSION['imsrt']['twitterMisc']['id'];
			}
