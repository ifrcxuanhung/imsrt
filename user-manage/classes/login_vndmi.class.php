...
class Login extends jigowatt_integration {
	...
	function __construct() {
		...
		if ( parent::getOption('signin-redirect-referrer-enable') )
			$_SESSION['imsrt']['referer'] = (!empty($_SESSION['imsrt']['referer'])) ? $_SESSION['imsrt']['referer'] : 'home.php';
		else
			$_SESSION['imsrt']['referer'] = parent::getOption('signin-redirect-url');
		...
		if(isset($_POST['login'])) :
			$this->user = parent::secure(isset($_POST['username']) ? $_POST['username'] : $_SESSION['imsrt']['username'] );
			$this->pass = parent::secure(isset($_POST['password']) ? $_POST['password'] : $_SESSION['imsrt']['password'] );
		...
		foreach (parent::$socialLogin as $provider) :
			if (!empty($_SESSION['imsrt'][$provider])) {
	...
	private function social_login($provider) {
		...
		$params = array( ':session' => $_SESSION['imsrt'][$provider] );
	...
	public function login() {
		...
		if ( parent::getOption('user-activation-enable') ) :
			...
			if ($count > 0) $_SESSION['imsrt']['activate'] = 1;

		endif;

		// Save if user is restricted
		if ( !empty($this->result['restricted']) ) $_SESSION['imsrt']['restricted'] = 1;
		...
		if (parent::getOption('pw-encrypt-force-enable')) :
			...
			if (strlen($this->result['password']) == 32 && $type == 'SHA256')
				$_SESSION['imsrt']['forcePwUpdate'] = 1;
		...
		$_SESSION['imsrt']['user_level'] = $user_level;

		$_SESSION['imsrt']['email'] = $this->result['email'];

		$_SESSION['imsrt']['gravatar'] = parent::get_gravatar($this->result['email'], true, 26);
		...
		if ( !empty($disRow['level_disabled']) ) $_SESSION['imsrt']['level_disabled'] = 1;
		...
		$query = parent::query("SELECT * FROM `login_confirm` WHERE `username` = '$user'");
		$queryRow =$query->fetch();
		//echo "<pre>";print_r($queryRow);exit;
		if(count($queryRow) > 1){
			$response['status'] = false;	
			echo json_encode($response);
			exit();
		...
		$_SESSION['imsrt']['username'] = $this->result[$username];

		// User ID of the logging in user
		$_SESSION['imsrt']['user_id'] = $this->result['user_id'];
		//$_SESSION['imsrt']['level_level'] = $this->result['level_level'];

		if ( empty($redirect) ) $redirect = $_SESSION['imsrt']['referer'];

		unset(
			$_SESSION['imsrt']['referer'],
			$_SESSION['imsrt']['token'],
			$_SESSION['imsrt']['facebookMisc'],
			$_SESSION['imsrt']['twitterMisc'],
			$_SESSION['imsrt']['openIDMisc']
		);
		...
		//$response['status'] = $this->result[$username];
		echo json_encode($response);
		exit();
