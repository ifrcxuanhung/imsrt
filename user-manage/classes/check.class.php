...
class Check extends Generic {
	...
	private function isGuest($forceLogin) {
		...
		if ( !$forceLogin )
			return empty( $_SESSION['imsrt']['user_id'] );

		if ( empty($_SESSION['imsrt']['user_id']) ) :
			...
			$_SESSION['imsrt']['referer'] = $_SERVER['REQUEST_URI'];
	...
	private function isActivated() {
		...
		if ( !empty($_SESSION['imsrt']['activate']) ) :
	...
	private function isRestricted() {
		...
		if ( !empty($_SESSION['imsrt']['restricted']) || !empty($_SESSION['imsrt']['level_disabled']) ) :
	...
	private function forcePwUpdate() {
		...
		if ( !empty($_SESSION['imsrt']['forcePwUpdate']) && basename($_SERVER['PHP_SELF']) != 'profile.php') :
	...
	public function protectPage($level) {
		...
		*/
		...
		if( ! @array_intersect($level, $_SESSION['imsrt']['user_level']) && $level != array('*') )
			$this->deny_access();
	...
	public function protectThis($level) {
		...
		*/
		...
		if ( empty( $_SESSION['imsrt']['user_id'] ) && $level == array('*') )
			return false;
