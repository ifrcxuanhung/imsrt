...
class Generic extends Connect {
	...
	public function denyAccessLogs() {
		...
		return ( ($this->getOption('profile-timestamps-admin-enable') && !in_array(1, $_SESSION['imsrt']['user_level'])) || !$this->getOption('profile-timestamps-enable') );
	...
	public function guestOnly() {
		...
		if(!empty($_SESSION['imsrt']['username'])) {
			$this->error =	"
