...
class Jigowatt_integration extends Generic {
	...
	private function retrieveInfo() {
		...
		$params = array( ':user_id' => $_SESSION['imsrt']['user_id'] );
	...
	public function link_account( $link, $login = false ) {
		...
		try{
			...
			if ( !empty( $provider_user_profile ) ) {
				$_SESSION['imsrt'][$link . 'Misc'] = (array) $provider_user_profile;
				$_SESSION['imsrt'][$link] = $provider_user_profile->identifier;
			}

			/** See if the link was successful. */
			if ( !empty( $_SESSION['imsrt'][$link] ) ) {
	...
	private function connect( $link ) {
		...
		$params = array(
			':user_id'      => $_SESSION['imsrt']['user_id'],
			':session_link' => $_SESSION['imsrt'][$link]
		);
