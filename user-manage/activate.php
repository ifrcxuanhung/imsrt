...
class Activate extends Generic {
	...
	function __construct() {
		...
		if(isset($_SESSION['imsrt']['username']))
			$this->user = $_SESSION['imsrt']['username'];
	...
	private function getKey() {
		...
		if ($stmt->rowCount() < 1) {
			$this->error = '<div class="alert alert-danger">' . _('Your activation link is incorrect.') . '</div>
					  ...
					  <p>' . sprintf(_('Go to the <a href="http://vndmi.com"> homepage</a>'), 'home.php') . '</p>';
			return false;
		}
		...
		if(!empty($_SESSION['imsrt']['activate'])) unset($_SESSION['imsrt']['activate']);
